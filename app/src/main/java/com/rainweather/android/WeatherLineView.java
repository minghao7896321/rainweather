...
public class WeatherLineView extends View {
     ...
     * 默认最小宽度50dp
     */
    private static final int defaultMinWidth = 70;

    /**
     * 默认最小高度120dp
     */
    private static final int defaultMinHeight = 120;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mWeaTextColor = a.getColor(R.styleable.WeatherLineView_weatextColor, Color.WHITE);
    ...
    private void initPaint() {
        mTextPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG); // 使位图抗锯齿的标志
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 最高温度和最低温度映射到这边的view的高度换算
        // view的起始高度 + ( ( 当天的温度 - 最低温度 ) * ( view的最高的高度 - 最低的高度 ) / ( 最高温度 - 最低温度 ) )
        // 这里view的最低高度是需要从文字的高度以上开始算的
        // 这里view的最高高度是需要减去文字的高度以下
        ...
        // 下边的线
        // TODO: 2016/5/30 颜色要改回来
        mLinePaint.setColor(Color.parseColor("#03f2ff"));
        if (mLowTemperData[0] != 0) {
            // 最低温度左边
            int calowLeft = baseHeight - cacHeight(mLowTemperData[0]);
            canvas.drawLine(0, calowLeft, getWidth() / 2, calowMiddle, mLinePaint); //void drawLines (float[] pts, int offset, int count, Paint paint)
        }
